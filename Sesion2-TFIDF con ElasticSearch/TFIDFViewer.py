...
def normalize(tw):
    ...
    :return:
    """
    valores = list(tw.values())
    valores = valores/np.linalg.norm(valores)
    llaves = list(tw.keys())
    for i in range(len(valores)):
        tw[llaves[i]] = valores[i]
    return tw
...
def cosine_similarity(tw1, tw2):
    ...
    :return:
    """
    # recorremos los 2 vectores para que ambdos tengan los mismos elementos
    # luego ordenamos y aplicamos las formulas explicadas en clase
    for w in tw2:
        if w not in tw1:
            tw1[w] = 0
    for w in tw1:
        if w not in tw2:
            tw2[w] = 0
    tw1 = dict(sorted(tw1.items()))
    tw2 = dict(sorted(tw2.items()))
    list1 = list(tw1.values())
    list2 = list(tw2.values())
